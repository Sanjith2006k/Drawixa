...
export function Dashboard() {
  ...
  const handleJoin = async (e) => {
    ...
    if (!joinCode) return;
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/boards/join`, {
        method: 'POST',
  ...
  const handleDelete = async (e, boardId) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/boards/${boardId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${user.token}` }
      });
